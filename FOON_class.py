class Object:
    ...
    def getObject(self):
        a=""
        a=a+"O" + str(self.getObjectType()) + "\t" + self.getObjectLabel()
        for x in range(len(self.getStatesList())):
            if "contains" in self.getStateLabel(
                x
            ) or "ingredients" in self.getStateLabel(x):
                a=a+"S"+ str(self.getStateType(x))+ "\t"+ self.getStateLabel(x)+ "\t"+ self.getIngredientsText()
            else:
                
                a=a+ "S" + str(self.getStateType(x)) + "\t" + self.getStateLabel(x)+ (str("\t " + "[" + str(self.getContainer(x)) + "]")
                        if self.getContainer(x)
                        else ""
                    )
